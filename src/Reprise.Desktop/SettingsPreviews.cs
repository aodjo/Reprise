...
public sealed class PanelTimePreview : Border
{
    ...
    public PanelTimePreview()
    {
        ...
        _slider = new CompactSlider { Minimum = 0, Maximum = PreviewDuration, Value = 69 };
        _slider.UserValueChanged += (_, _) => UpdateTimes();
        _slider.Bind(CompactSlider.TrackBrushProperty, _slider.GetResourceObservable("SettingsWellBrush"));
        _slider.Bind(CompactSlider.FillBrushProperty, _slider.GetResourceObservable("SettingsAccentBrush"));
        _leading = new TextBlock { FontSize = PanelTypography.Caption };
        _trailing = new TextBlock { FontSize = PanelTypography.Caption, HorizontalAlignment = HorizontalAlignment.Right };
        _leading.Bind(TextBlock.ForegroundProperty, _leading.GetResourceObservable("SettingsSecondaryBrush"));
        _trailing.Bind(TextBlock.ForegroundProperty, _trailing.GetResourceObservable("SettingsSecondaryBrush"));
        var times = new Grid { ColumnDefinitions = new ColumnDefinitions("*,Auto"), Children = { _leading, _trailing } };
        Grid.SetColumn(_trailing, 1);
